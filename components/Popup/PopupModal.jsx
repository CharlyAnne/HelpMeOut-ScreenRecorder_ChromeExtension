...
const RecordModal = () => {
  ...
  return (
    <main>
      <section className="shadow-2xl rounded-2xl space-y-6 w-[410px] p-6">
        ...
        <div className="flex justify-center gap-8">
          {data.map((item) => (
            <section
              key={item.id}
              onClick={() => setActiveScreenType(item.name)}
              className={`flex cursor-pointer flex-col items-center gap-2 ${
                activeScreenType === item.name
                  ? 'text-[#120B48]'
                  : 'text-[#928FAB]'
              }`}
            >
        ...
        <div className="">
          <button className="bg-[#120B48] text-white rounded-[10px] w-full p-4">
            Start Recording
          </button>
        </div>
      </section>
